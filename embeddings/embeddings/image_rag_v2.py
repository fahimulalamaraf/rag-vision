...
def expand_query_with_gemini(query_text: str) -> str:
    ...
    response = chat_session.send_message([query_text, "Please make this query clear, short and make sense."])
    expanded_query = response.text
    print(expanded_query)
    return expanded_query

def make_sense_with_gemini(query_text: str) -> str:
    """Uses Gemini to expand the text query and return the expanded version."""
    ...
    response = chat_session.send_message([query_text, "Please make a sensible text query from this. No preamble."])
    expanded_query = response.text
    return expanded_query
...
def describe_image_with_gemini(image_path: str) -> str:
    ...
    chat_session = gemini_model.start_chat(history=[])
    response = chat_session.send_message([file, "Please describe this image in 4 words."])
...
def hybrid_search_with_description(query_text: str, query_image_path: str) -> torch.Tensor:
    """
    Performs a hybrid search using both text and image queries, where the image is used to generate a description
    ...
    image_description = describe_image_with_gemini(query_image_path)
    print(f"Generated image description: {image_description}")


    # Combine the text query with the image description
    combined_query = f"{query_text} {image_description}"
    print(f"Generated query description: {combined_query}")

    final_query = make_sense_with_gemini(combined_query)
    print(final_query)

    final_embedding = get_text_embedding(final_query)
...
if operation_choice == "1":
    ...
    if query_input.lower().endswith(('.jpg', '.jpeg', '.png')):
        ...
        expanded_query = expand_query_with_gemini(query_input)      # expand function not working
        print(f"Expanded Query: {query_input}")
    ...
    query_text = input("Enter your query text: ").strip()
    query_image_path = input("Enter the path to an image: ").strip()
    query_image_path_hybrid = os.path.join(image_dir, query_image_path)
    top_k_results = hybrid_search_with_description(query_text, query_image_path_hybrid)

"""distances, indices = index.search(top_k_results.reshape(1, -1), 5)
    indices_distances = list(zip(indices[0], distances[0]))
    for idx, distance in indices_distances:
        print(f"Index: {idx}, Distance: {distance}")
        image_path = image_paths[idx]
